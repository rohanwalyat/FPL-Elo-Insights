...
market_odds AS (
    ...
    FROM {{ ref('stg_odds__h2h') }} h
    JOIN {{ ref('dim_teams') }} t_home ON h.home_team = t_home.team_name
    JOIN {{ ref('dim_teams') }} t_away ON h.away_team = t_away.team_name
),
...
fixture_intelligence AS (
    ...
    JOIN {{ ref('dim_teams') }} t ON u.opponent_id = t.team_id
...
draft_players AS (
    SELECT * FROM {{ ref('dim_draft_players') }}
),
...
ml_preds AS (
    SELECT * FROM {{ ref('stg_ml__projections') }}
),
